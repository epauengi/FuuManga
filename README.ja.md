...
│   ├── Reader.jsx          # 縦スクロール型連続リーダーコンポーネント
│   ├── core.mjs            # 正規化、ルーティングパーサー、状態バリデーション
│   ├── sources.js          # MangaDex および OTruyen API アダプター・キャッシュ
│   ├── styles.css          # デザインシステム、テーマ定義、レスポンシブスタイル
│   └── main.jsx            # アプリケーションのエントリーポイント
├── tests/
│   ├── core.test.mjs       # 単体テスト (Node.js test runner)
